...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    ...
    [self setVisibleComponentsAnimated:YES];
}
...
//        self.url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.instapaper.com/text?u=%@", self.baseUrl]];
        self.url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.instapaper.com/text?u=%@", [[self.url absoluteString] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
    }
    self.textOnlyView = !self.textOnlyView;
    [self setShowAddressBar:!self.textOnlyView animated:YES];
...
//    pinbookActivity.bookmarkParameters = @{ALBPinbookTitleParameterKey : [self.delegate titleToShare]};
    ...
    NSArray *activityItems = @[self.url, titleLabel.text];
//    NSArray *appActivities = @[chromeActivity, instapaperActivity, safariActivity, pinbookActivity];
    ...
    if (!self.activityPopover.isPopoverVisible) {
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
            ...
            [self presentViewController:activityVC animated:TRUE completion:NULL];
        }
    } else {
